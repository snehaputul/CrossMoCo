...
class ModelMoCo(nn.Module):
    ...
    def contrastive_loss(self, data_t1, data_t2, new_imgs):
        ...
        with torch.no_grad():  # no gradient to keys
            # shuffle for making use of BN
            key_images_features = []
            for img in new_imgs[1:]:
                data_t2_, idx_unshuffle = self._batch_shuffle_single_gpu(img)
                ...
                key_images_features.append(k_img)
            # average the key features from different images
            k_img = torch.mean(torch.stack(key_images_features), dim=0)
...
def train(args, io):
    ...
    train_loader = DataLoader(ShapeNetRender(transform, n_imgs=args.num_images), num_workers=10,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_val_loader = DataLoader(ModelNet40SVM(partition='train', num_points=1024), batch_size=20, shuffle=True)
        test_val_loader = DataLoader(ModelNet40SVM(partition='test', num_points=1024), batch_size=20, shuffle=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--num_images', type=int, default=2, help='number of rendered images')
