...
def get_render_imgs(pcd_path):
    spilitter = '/' if '/' in pcd_path else '\\'
    path_lst = pcd_path.split(spilitter)
...
class ShapeNetRender(Dataset):
    ...
    def __getitem__(self, item):  # returns data
        ...
        render_img_list = []
        for path in render_img_path_list:
            render_img = Image.open(path).convert('RGB')
        ...
        return pointcloud, render_img_list  # render_img_list
